...
IN1  = ROOT / "data" / "intraday.csv"
OUT_MATCHED = ROOT / "data" / "intraday_matched_psm2.csv"
OUT_UPDATED = ROOT / "data" / "intraday_psm.csv"
...
TARGET_TREATED = 1223                          # expected treated count
...
def main():
    ...
    before = len(df)
    df = df.dropna(subset=["psm_group"]).copy()
    after = len(df)
    print(f"[info] Dropped {before - after:,} rows without psm_group (kept {after:,})")
    df = df.dropna(subset=["psm_group"]).copy()
    df.to_csv(OUT_UPDATED, index=False)
    print(f"[done] Updated intraday dataset with 'psm_group' dummy no (NAs) → {OUT_UPDATED}")

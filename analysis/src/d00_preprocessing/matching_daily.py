...
TARGET_TREATED = 448                           # expected treated count
...
def load_daily():
    ...
    if "date" in df.columns:
        ...
        raise ValueError("No 'date' or 'datetime' column in input file.")
    return df, dtcol
...
def main():
    ...
    if len(treated) != n_treat:
        print(f"[warn] expected 448 treated, found {len(treated)}. Using {n_treat}.")
    ...
    print(f"[done] Updated daily dataset with 'psm_group' dummy → {OUT_UPDATED}")

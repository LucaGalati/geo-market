...
for (sample in c(SAMPLES, INTERNAL)) {
  ...
  write_tex(rows, c("", "N", "Mean", "SD", "P25", "Median", "P75"), "descriptive_stats", sample,
            ...
            notes = "This table reports the distribution of the variables used in the analysis. Panel A covers the whole window of 40 trading days, 27 January to 23 March 2022, Panel B the 20 trading days before the invasion and Panel C the 20 trading days on and after 24 February 2022; an observation is a firm-day. The table of the main text uses the matched sample of pairs of a Nearby and a Distant firm (Section~\\ref{sec:matching}) and the appendices repeat it on the full sample (\\ref{sec:appendixC}) and on the sample matched on market value and price (\\ref{sec:appendixE}). The quoted spread, the effective spread, the price impact and the realized spread are in percent of the midpoint, the effective spread and its two components being half-spreads; dollar volume is in logs of U.S. dollars and the number of trades in logs; volatility is the daily standard deviation of the five-minute midpoint returns, in percent; market value is in logs of U.S. dollars. Each daily variable is winsorized at the 1st and 99th percentiles within the sample under analysis. \\ref{sec:appendixA} gives the complete variable definitions.")

...
for (sample in SAMPLES) {
  ...
  write_tex(rows, c("Day", "Nearby", "Distant", "Difference", "SE", "Firm-days"), "intraday_difference", sample,
            ...
            notes = "This table reports the standardized abnormal quoted spread of the Nearby firms, of the Distant firms and the difference between the two, by trading day around the invasion (day 0 is 24 February 2022). For each firm and day the abnormal spread is the mean quoted spread over the closing hour for days $-5$ to $-1$ and over the opening hour for days 0 to $+5$, standardized with the mean and the standard deviation of the same hour in the same group over the benchmark window of days $-20$ to $-6$, so that closing hours are standardized on closing hours and opening hours on opening hours. Nearby firms are headquartered in a country bordering Ukraine or in a country bordering one of those, Distant firms elsewhere; the table of the main text uses the matched sample of pairs (Section~\\ref{sec:matching}) and that of Appendix~\\ref{sec:appendixC} the full sample. Difference is the coefficient of Nearby $\\times$ day in a regression of the standardized spreads on day fixed effects and their interactions with the Nearby indicator; the standard errors in the SE column are independently clustered at the level of both firms and days \\citep{CameronGelbachMiller2011}, and $p$-values are denoted as * $p<0.05$, ** $p<0.01$, *** $p<0.001$. Firm-days is the number of firm-day observations behind the row, Nearby and Distant together; no winsorization is applied. See Appendix~\\ref{sec:appendixA} for the construction of the standardized abnormal spreads.")
}

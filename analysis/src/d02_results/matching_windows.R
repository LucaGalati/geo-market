...
# (psm_core.WINDOWS, files psm_assignments_<tag>.parquet): the appendix table and a csv
# with every estimate (four outcomes x four exposure measures, with and without controls).
# The window of the main text is psm_core.MAIN_WINDOW, fixed by the design of the
# matching, not selected here.
...
cat(sprintf("[windows] positive and significant: %s\n", paste(sprintf("%s=%d/32", names(res), sapply(res, function(e) sum(e$coef > 0 & e$p < 0.05))), collapse = ", ")))

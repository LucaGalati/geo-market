...
def stage_phase_one(
    ...
    *,
    ...
    tmp_root/<base>/ric=<RIC>/date_local=<YYYY-MM-DD>/part-XXXX.(csv.gz|parquet)
    """
...
def consolidate_phase_two(
    ...
    *,
    ...
    """Phase 2: for each ric/date_local shard, compute metrics & append to final output."""
    ...
    def _append_output(agg: pd.DataFrame):
...
def _build_arg_parser():
    ...
    p.add_argument("--temp-partition-cols", default="ric,date_local")
    p.add_argument("--temp-parquet-compression", default=DEFAULT_PARQUET_COMPRESSION)
    p.add_argument("--temp-parquet-max-rows", type=int, default=DEFAULT_PARQUET_MAX_ROWS)
    ...
    p.add_argument("--phase2-workers", type=int, default=1)
...
def main():
    ...
    def _run():
        ...
        log(f"Phase 2 workers: {max(1, args.phase2_workers)}")
        ...
        for gz in gz_files:
            ...
            with timed(f"Phase 1 total ({gz.name})", args.timings):
                staged_root = stage_phase_one(
                    ...
                    parquet_max_rows=args.temp_parquet_max_rows,
